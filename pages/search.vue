<template>
<div id="content" class="container-fluid">
  <div class="container">
    <div id="blog-nav" class="f-c">
      <div class="search-title f-c v-a-c">
        <div class="v-a-b">
          <div class="h2">
            为例查询到“恩恩”
          </div>
        </div>
      </div>
      <div id="search" class="v-a-c">
        <div class="v-a-b">
          <form action="https://fireart.studio/blog" type="GET">
            <div class="v-a-b">
              <input type="text" name="s" placeholder="搜索">
              <div class="search-icon">
                <svg version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                x="0px" y="0px" viewBox="0 0 25 25" style="enable-background:new 0 0 25 25;"
                xml:space="preserve">
                  <path id="XMLID_3_" class="st0" d="M16.5,18.2c-0.5-0.5-0.5-1.3,0-1.8c0.5-0.5,1.3-0.5,1.8,0l5.3,5.3c0.5,0.5,0.5,1.3,0,1.8
                  c-0.5,0.5-1.3,0.5-1.8,0L16.5,18.2z">
                  </path>
                  <circle id="XMLID_5_" class="st1" cx="10.3" cy="10.5" r="8.5">
                  </circle>
                </svg>
              </div>
            </div>
          </form>
        </div>
      </div>
    </div>
    <div id="content-area">
      <div id="ajax">
        <div class="h3 not-found">
          请求没有内容，换个关键字试试！
        </div>
        <div class="posts f-c">
        </div>
      </div>
    </div>
  </div>
</div>
</template>

<style scoped>
  #content{
    background: #fff
  }
</style>